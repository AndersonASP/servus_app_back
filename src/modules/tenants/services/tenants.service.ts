...
export class TenantService {
  ...
  async create(createTenantDto: CreateTenantDto, createdBy: string) {
    try {
      ...
      console.log(
        `✅ [TenantService] Tenant "${createTenantDto.name}" criado com sucesso`,
      );
      return savedTenant;
    } catch (error) {
  ...
  async createWithAdmin(
    ...
    try {
      ...
      try {
        ...
        console.log(
          `✅ [TenantService] Tenant "${data.tenantData.name}" criado com sucesso${adminResult ? ' com administrador' : ''}`,
        );
  ...
  async debugFunctionLookup(tenantId: string, userId: string) {
    ...
    try {
      ...
      console.log(
        `🔍 Encontradas ${memberFunctions.length} MemberFunctions aprovadas`,
      );
  ...
  async debugMemberFunctionsAfterApproval(tenantId: string, userId: string) {
    ...
    try {
      ...
      const memberFunctions = await memberFunctionModel
        .find({
          memberId: new Types.ObjectId(userId),
          ministryId: membership.ministry,
        })
      ...
      return {
        ...
        totalFunctions: memberFunctions.length,
        approvedFunctions: memberFunctions.filter(
          (mf) => mf.status === 'aprovado',
        ).length,
        pendingFunctions: memberFunctions.filter(
          (mf) => mf.status === 'pending',
        ).length,
      };
    } catch (error) {
  ...
  async getPendingVolunteers(tenantId: string, ministryId?: string) {
    ...
    try {
      ...
      if (ministryId && ministryId.trim() !== '') {
        ...
        console.log(
          '🔍 [TenantService] Filtrando por ministério específico:',
          ministryId,
        );
        console.log(
          '🔍 [TenantService] Query final com filtro:',
          JSON.stringify(baseQuery, null, 2),
        );
      } else {
        console.log(
          '🔍 [TenantService] Nenhum ministryId fornecido ou vazio, retornando todos os ministérios',
        );
        console.log(
          '🔍 [TenantService] Query final sem filtro:',
          JSON.stringify(baseQuery, null, 2),
        );
      }
      ...
      const memberships = await this.membershipModel
        ...
        .populate(
          'user',
          '_id name email phone role isActive createdAt profileCompleted',
        )
        .populate('ministry', 'name')
      ...
      console.log(
        `✅ Encontrados ${memberships.length} voluntários pendentes de aprovação`,
      );

      // Converter para formato esperado pelo frontend
      const pendingVolunteers = memberships
        .map((membership) => {
          const user = membership.user as any;

          console.log(
            '🔍 [TenantService] Processando membership:',
            membership._id,
          );
          ...
          const volunteerData = {
            ...
            ministry: {
              ...
              name:
                (membership.ministry as any)?.name ||
          ...
          console.log(
            '🔍 [TenantService] Volunteer data criado:',
            volunteerData,
          );
      ...
      console.log(
        '✅ Voluntários pendentes convertidos para formato do frontend',
      );
  ...
  async getMinistryFunctions(tenantId: string, ministryId: string) {
    ...
    try {
      ...
      console.log(
        '🔍 [TenantService] Modelo criado:',
        ministryFunctionModel.modelName,
      );
      console.log(
        '🔍 [TenantService] Collection name:',
        ministryFunctionModel.collection.name,
      );
      ...
      console.log(
        '🔍 [TenantService] Ministry ObjectId string:',
        ministryObjectId.toString(),
      );

      // Primeiro, vamos ver todas as funções da collection
      const allFunctions = await ministryFunctionModel
        .find({})
        .select('name description level ministryId isActive')
        .sort({ name: 1 });
      console.log(
        '🔍 [TenantService] TODAS as funções na collection:',
        allFunctions.length,
      );
      console.log(
        '🔍 [TenantService] Exemplos de funções:',
        allFunctions.slice(0, 3),
      );

      // Vamos ver a estrutura completa de uma função
      if (allFunctions.length > 0) {
        console.log(
          '🔍 [TenantService] ESTRUTURA COMPLETA da primeira função:',
        );
        ...
        console.log(
          '   - ministryId type:',
          typeof (allFunctions[0] as any).ministryId,
        );
      ...
      console.log(
        '🔍 [TenantService] Tentando query 1: ministryId como ObjectId com populate',
      );
      const functions1 = await ministryFunctionModel
        .find({
          ministryId: ministryObjectId,
          isActive: true,
        })
        .populate('functionId', 'name description level')
        .sort({ 'functionId.name': 1 });
      console.log('🔍 [TenantService] Resultado query 1:', functions1.length);
      if (functions1.length > 0) {
        console.log(
          '🔍 [TenantService] Primeira função query 1:',
          functions1[0],
        );
        console.log(
          '🔍 [TenantService] - functionId:',
          (functions1[0] as any).functionId,
        );
        console.log(
          '🔍 [TenantService] - functionId type:',
          typeof (functions1[0] as any).functionId,
        );
      }

      console.log(
        '🔍 [TenantService] Tentando query 2: ministryId como string',
      );
      const functions2 = await ministryFunctionModel
        .find({
      ...
      console.log(
        '🔍 [TenantService] Tentando query 3: ministryId como ObjectId sem isActive',
      );
      const functions3 = await ministryFunctionModel
        .find({
          ministryId: ministryObjectId,
        })
        .select('name description level')
      ...
      console.log(
        '🔍 [TenantService] Tentando query 4: sem filtro de ministryId',
      );
      const functions4 = await ministryFunctionModel
        .find({})
        .select('name description level ministryId')
        .sort({ name: 1 });
      ...
      if (functions4.length > 0) {
        ...
        console.log(
          '   - ministryId type:',
          typeof (functions4[0] as any).ministryId,
        );
      }
  ...
  async approveVolunteer(
    ...
    try {
      ...
      const membership = await this.membershipModel.findOne({
        user: new Types.ObjectId(userId),
        tenant: new Types.ObjectId(tenantId),
        $or: [{ isActive: false }, { needsApproval: true }],
      });
      ...
      console.log(
        '🔍 [TenantService] Final Function IDs length:',
        finalFunctionIds.length,
      );
      console.log('🔍 [TenantService] Membership source:', membership.source);

      if (membership.source === 'invite' && finalFunctionIds.length === 0) {
        console.log(
          '❌ [TenantService] ERRO: Voluntário de invite sem funções selecionadas',
        );
        throw new BadRequestException(
          'Para voluntários vindos de código de convite, é obrigatório atribuir pelo menos uma função',
        );
      }
      ...
      try {
        ...
        if (membership.source === 'invite' && finalFunctionIds.length > 0) {
          ...
          for (const funcId of finalFunctionIds) {
            try {
              ...
              console.log(
                '🔧 Criando MemberFunction com dados:',
                memberFunctionData,
              );
              ...
              console.log(
                '✅ MemberFunction criada e aprovada:',
              ...
              console.error(
                '❌ Erro ao criar MemberFunction para função',
                funcId,
                ':',
                error.message,
              );
          ...
          console.log(
            `🔍 Encontradas ${existingMemberFunctions.length} MemberFunctions pendentes`,
          );
          existingMemberFunctions.forEach((mf, index) => {
            console.log(
              `   - ${index + 1}: ${mf._id} - Function: ${mf.functionId} - Status: ${mf.status}`,
            );
          });
          ...
          if (existingMemberFunctions.length > 0) {
            ...
            console.log(
              `✅ MemberFunctions do formulário aprovadas: ${updateResult.modifiedCount} atualizadas`,
            );
          } else {
            console.warn(
              '⚠️ Nenhuma MemberFunction pendente encontrada para voluntário de formulário',
            );
            console.log(
              '🔍 Tentando buscar MemberFunctions com outros status...',
            );
            ...
            console.log(
              `🔍 Total de MemberFunctions encontradas: ${allMemberFunctions.length}`,
            );
            allMemberFunctions.forEach((mf, index) => {
              console.log(
                `   - ${index + 1}: ${mf._id} - Function: ${mf.functionId} - Status: ${mf.status} - Active: ${mf.isActive}`,
              );
            });
          }
        }
        ...
        console.warn(
          '⚠️ Erro ao gerenciar MemberFunctions:',
          memberFunctionError.message,
        );
  ...
  async rejectVolunteer(
    ...
    try {
      ...
      try {
        ...
        console.log(
          `✅ MemberFunctions rejeitadas: ${updateResult.modifiedCount} atualizadas`,
        );
      } catch (memberFunctionError) {
        console.warn(
          '⚠️ Erro ao rejeitar MemberFunctions:',
          memberFunctionError.message,
        );
      }
  ...
  async debugVolunteers(tenantId: string) {
    ...
    try {
      ...
      console.log(
        `🔍 [DEBUG] Encontrados ${memberships.length} memberships brutos`,
      );
  ...
  async debugMinistryFunctions(tenantId: string, ministryId: string) {
    ...
    try {
      ...
      const { MinistrySchema } = await import(
        '../../ministries/schemas/ministry.schema'
      );
      const ministryModel = this.membershipModel.db.model(
        'Ministry',
        MinistrySchema,
      );
      ...
      const allFunctions = await ministryFunctionModel
        .find({})
        .select('name description level ministry isActive')
        .sort({ name: 1 });
      console.log(
        '🔍 [DEBUG] TODAS as funções na collection:',
        allFunctions.length,
      );
      console.log('🔍 [DEBUG] Exemplos de funções:', allFunctions.slice(0, 5));

      // 3. Verificar funções específicas do ministério
      const ministryFunctions = await ministryFunctionModel
        .find({
      ...
      const activeFunctions = await ministryFunctionModel
        ...
        .sort({ name: 1 });
      console.log(
        '🔍 [DEBUG] Funções ativas do ministério:',
  ...
  async debugMemberFunctions(tenantId: string, userId: string) {
    ...
    try {
      ...
      const allMemberFunctions = await memberFunctionModel
        .find({
          memberId: new Types.ObjectId(userId),
          tenantId: new Types.ObjectId(tenantId),
        })
      ...
      console.log(
        '🔍 [DEBUG] MemberFunctions encontradas:',
        allMemberFunctions.length,
      );
      console.log('🔍 [DEBUG] MemberFunctions:', allMemberFunctions);

      // 3. Verificar MemberFunctions pendentes
      const pendingMemberFunctions = await memberFunctionModel
        .find({
          memberId: new Types.ObjectId(userId),
          tenantId: new Types.ObjectId(tenantId),
          status: 'pending',
      ...
      console.log(
        '🔍 [DEBUG] MemberFunctions pendentes:',
        pendingMemberFunctions.length,
      );

      // 4. Verificar MemberFunctions aprovadas
      const approvedMemberFunctions = await memberFunctionModel
        .find({
          memberId: new Types.ObjectId(userId),
          tenantId: new Types.ObjectId(tenantId),
          status: 'aprovado',
      ...
      console.log(
        '🔍 [DEBUG] MemberFunctions aprovadas:',
        approvedMemberFunctions.length,
      );
