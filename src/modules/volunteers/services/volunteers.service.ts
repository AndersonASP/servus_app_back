...
import { FormSubmission, FormSubmissionStatus } from 'src/modules/forms/schemas/form-submission.schema';
...
export class VolunteersService {
  ...
  async list(tenantId: string, user: any, q: GetVolunteersDto) {
    ...
    console.log('🔍 [VolunteersService] user:', { _id: user._id, role: user.role, email: user.email });
    ...
    const membershipVolunteers = await this.getVolunteersFromMemberships(tenantOid, scope, q);

    console.log('🔍 [VolunteersService] membershipVolunteers encontrados:', membershipVolunteers.length);
    ...
    console.log('🔍 [VolunteersService] dados paginados:', paginatedData.length);
  ...
  private async getVolunteersFromMemberships(tenantOid: Types.ObjectId, scope: any, q: GetVolunteersDto) {
    console.log('🔍 [VolunteersService] Iniciando getVolunteersFromMemberships');
    console.log('🔍 [VolunteersService] scope:', scope);
    ...
    console.log('🔍 [VolunteersService] matchQuery:', JSON.stringify(matchQuery, null, 2));
      ...
      { $unwind: '$ministry' },
      {
        $lookup: {
          ...
          pipeline: [
            ...
            { $unwind: { path: '$functionData', preserveNullAndEmptyArrays: false } }, // ❌ NÃO preservar se função não existir
            {
              $project: {
                _id: 0,
                id: '$functionData._id',
    ...
    console.log('🔍 [VolunteersService] Pipeline simplificado:', JSON.stringify(pipeline, null, 2));
    
    const results = await this.memModel.aggregate(pipeline).allowDiskUse(true);
    console.log('🔍 [VolunteersService] Resultados encontrados:', results.length);
    
    // Debug: verificar funções encontradas
    results.forEach((volunteer, index) => {
      console.log(`🔍 [VolunteersService] Voluntário ${index + 1}: ${volunteer.name}`);
      ...
      if (volunteer.functions && volunteer.functions.length > 0) {
        volunteer.functions.forEach((func, funcIndex) => {
          ...
          console.log(`       - FunctionDataExists: ${func.functionDataExists ? 'SIM' : 'NÃO'}`);
  ...
  private async getVolunteersFromForms(tenantOid: Types.ObjectId, scope: any, q: GetVolunteersDto, pendingOnly: boolean = false) {
    ...
    if (q.ministryId) matchBase.preferredMinistry = new Types.ObjectId(q.ministryId);
    ...
    const pipeline: any[] = [
      ...
      { $unwind: '$ministry' },
      {
        $project: {
          _id: 0,
          userId: '$_id',
          name: { $ifNull: ['$volunteerName', '$customFields.volunteerName', '$customFields.name', 'Nome não informado'] },
    ...
    const results = await this.formSubmissionModel.aggregate(pipeline).allowDiskUse(true);
    return results;
  }
  ...
  async deleteVolunteer(tenantId: string, volunteerId: string, user: any) {
    ...
    console.log('🔍 [DeleteVolunteer] user:', { _id: user._id, role: user.role, email: user.email });
    ...
    try {
      ...
      console.log('🔍 [DeleteVolunteer] Erro ao converter volunteerId para ObjectId:', error);
    ...
    if (scope.isServusAdmin) {
      ...
      console.log('🔍 [DeleteVolunteer] Query:', JSON.stringify(query, null, 2));
      ...
      const query = {
        $or: [
          { _id: volunteerObjectId, tenant: tenantOid, ministry: { $in: ministryIds } },
          { user: userObjectId, tenant: tenantOid, ministry: { $in: ministryIds } }
        ]
      };
      console.log('🔍 [DeleteVolunteer] Query:', JSON.stringify(query, null, 2));
      volunteer = await this.memModel.findOne(query);
    ...
    if (!volunteer) {
      ...
      const alternativeSearch = await this.memModel.findOne({
        tenant: tenantOid,
        $or: [
      ...
      console.log('🔍 [DeleteVolunteer] Busca alternativa resultou em:', alternativeSearch);
      
      // Vamos também listar alguns memberships para debug
      const sampleMemberships = await this.memModel.find({
        tenant: tenantOid
      }).limit(3).lean();
      console.log('🔍 [DeleteVolunteer] Sample memberships:', sampleMemberships);
      
      // Vamos procurar especificamente por memberships de voluntários
      const volunteerMemberships = await this.memModel.find({
        tenant: tenantOid,
        role: 'volunteer'
      }).lean();
      console.log('🔍 [DeleteVolunteer] Volunteer memberships:', volunteerMemberships);
      
      throw new Error('Voluntário não encontrado ou sem permissão para excluir');
    }
    ...
    const memberFunctionModel = this.memModel.db.model('MemberFunction', MemberFunctionSchema);
    
    const deletedFunctionsCount = await memberFunctionModel.deleteMany({
      memberId: userId,
      tenantId: tenantOid
    });
    console.log(`✅ [DeleteVolunteer] ${deletedFunctionsCount.deletedCount} MemberFunctions removidas`);
    ...
    console.log(`✅ [DeleteVolunteer] ${deletedMembershipsCount.deletedCount} memberships removidos`);
    ...
    if (deletedUser) {
      console.log(`✅ [DeleteVolunteer] Usuário ${deletedUser.name} removido da base`);
    } else {
      console.log('⚠️ [DeleteVolunteer] Usuário não encontrado para exclusão');
    }

    console.log('🎉 [DeleteVolunteer] Exclusão completa realizada com sucesso!');
  ...
  async getPendingSubmissions(tenantId: string, user: any, q: GetVolunteersDto) {
    ...
    const pendingSubmissions = await this.getVolunteersFromForms(tenantOid, scope, q, true);

    console.log('🔍 [VolunteersService] submissões pendentes encontradas:', pendingSubmissions.length);
