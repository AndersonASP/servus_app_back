...
export class MinistryApprovalService {
  ...
  async getMinistryPendingSubmissions(
    ...
    this.logger.log(
      `[getMinistryPendingSubmissions] Query: ${JSON.stringify(query)}`,
    );
    ...
    this.logger.log(
      `[getMinistryPendingSubmissions] Encontradas ${submissions.length} submissões de ${total} total`,
    );
  ...
  async approveVolunteerSubmission(
    ...
    this.logger.log(
      `[approveVolunteerSubmission] Aprovando submissão ${submissionId} pelo usuário ${currentUserId}`,
    );
    ...
    if (!submission.preferredMinistry) {
      throw new BadRequestException(
        'Submissão não possui ministério preferido',
      );
    }
    ...
    if (approvalDto.status === 'approved') {
      this.logger.log(
        `[approveVolunteerSubmission] Criando usuário e membership para submissão aprovada`,
      );

      try {
        const { userId, membershipId } =
          await this.createUserAndMembershipFromSubmission(
            submission,
            tenantId,
            currentUserId,
          );
        ...
        this.logger.log(
          `[approveVolunteerSubmission] Usuário ${userId} e membership ${membershipId} criados com sucesso`,
        );
      } catch (createError) {
        this.logger.error(
          `[approveVolunteerSubmission] Erro ao criar usuário/membership: ${createError.message}`,
        );
        throw new BadRequestException(
    ...
    const updateData = {
      ...
      createdMembershipId: createdMembershipId
        ? new Types.ObjectId(createdMembershipId)
        : null,
    };
    ...
    this.logger.log(
      `[approveVolunteerSubmission] Submissão ${submissionId} ${approvalDto.status} com sucesso`,
    );

    // 🆕 Notificar voluntário sobre a decisão
    try {
      const leaderName =
        (updatedSubmission.approvedByLeader as any)?.name ||
        'Líder do Ministério';
      ...
      this.logger.error(
        `[approveVolunteerSubmission] Erro ao enviar notificação: ${notificationError.message}`,
      );
  ...
  private async validateMinistryLeaderAccess(
    ...
    this.logger.log(
      `[validateMinistryLeaderAccess] Validando acesso do usuário ${userId} ao ministério ${ministryId}`,
    );
    ...
    if (!membership) {
      this.logger.warn(
        `[validateMinistryLeaderAccess] Usuário ${userId} não é líder do ministério ${ministryId}`,
      );
      throw new ForbiddenException('Você não é líder deste ministério');
    }

    this.logger.log(
      `[validateMinistryLeaderAccess] Acesso validado para usuário ${userId}`,
    );
  }
  ...
  async getMinistryApprovalStats(
    ...
    const [pending, approved, rejected, total] = await Promise.all([
      this.formSubmissionModel.countDocuments({
        ...query,
        status: FormSubmissionStatus.PENDING,
      }),
      this.formSubmissionModel.countDocuments({
  ...
  async getLeaderPendingSubmissions(
    ...
    this.logger.log(
      `[getLeaderPendingSubmissions] Buscando submissões pendentes para líder ${leaderId}`,
    );
    ...
    if (leaderMemberships.length === 0) {
      this.logger.warn(
        `[getLeaderPendingSubmissions] Usuário ${leaderId} não é líder de nenhum ministério`,
      );
      return {
        submissions: [],
    ...
    this.logger.log(
      `[getLeaderPendingSubmissions] Ministérios do líder: ${ministryIds.length}`,
    );
    ...
    this.logger.log(
      `[getLeaderPendingSubmissions] Query: ${JSON.stringify(query)}`,
    );
    ...
    this.logger.log(
      `[getLeaderPendingSubmissions] Encontradas ${submissions.length} submissões pendentes`,
    );
  ...
  async getLeaderApprovalHistory(
    ...
    this.logger.log(
      `[getLeaderApprovalHistory] Buscando histórico do líder ${leaderId}`,
    );
  ...
  private async createUserAndMembershipFromSubmission(
    ...
    this.logger.log(
      `[createUserAndMembershipFromSubmission] Criando usuário para submissão ${submission._id}`,
    );
    ...
    if (existingUser) {
      this.logger.warn(
        `[createUserAndMembershipFromSubmission] Usuário já existe com email ${submission.email}`,
      );
    ...
    this.logger.log(
      `[createUserAndMembershipFromSubmission] Usuário criado: ${savedUser._id}`,
    );
    ...
    const membership = new this.membershipModel({
      ...
      role:
        submission.preferredRole === 'leader'
    ...
    this.logger.log(
      `[createUserAndMembershipFromSubmission] Membership criado: ${savedMembership._id}`,
    );

    // 5. Criar MemberFunctions para as funções selecionadas
    this.logger.log(
      `[createUserAndMembershipFromSubmission] Verificando funções selecionadas: ${JSON.stringify(submission.selectedFunctions)}`,
    );
    this.logger.log(
      `[createUserAndMembershipFromSubmission] Ministério preferido: ${submission.preferredMinistry}`,
    );
    this.logger.log(
      `[createUserAndMembershipFromSubmission] Tipo de selectedFunctions: ${typeof submission.selectedFunctions}`,
    );
    this.logger.log(
      `[createUserAndMembershipFromSubmission] Length de selectedFunctions: ${submission.selectedFunctions?.length}`,
    );
    this.logger.log(
      `[createUserAndMembershipFromSubmission] Tipo de preferredMinistry: ${typeof submission.preferredMinistry}`,
    );

    if (
      submission.selectedFunctions &&
      submission.selectedFunctions.length > 0 &&
      submission.preferredMinistry
    ) {
      this.logger.log(
        `[createUserAndMembershipFromSubmission] ✅ Condições atendidas - Criando ${submission.selectedFunctions.length} funções para o usuário`,
      );

      for (const functionName of submission.selectedFunctions) {
        try {
          this.logger.log(
            `[createUserAndMembershipFromSubmission] Buscando ID da função "${functionName}" para usuário ${savedUser._id}`,
          );

          // Buscar o ID da função pelo nome no ministério
          const ministryFunction =
            await this.memberFunctionService.findFunctionByNameInMinistry(
              functionName,
              submission.preferredMinistry._id.toString(),
              tenantId,
            );
          ...
          if (!ministryFunction) {
            this.logger.warn(
              `[createUserAndMembershipFromSubmission] Função "${functionName}" não encontrada no ministério ${submission.preferredMinistry._id}`,
            );
            continue;
          }

          this.logger.log(
            `[createUserAndMembershipFromSubmission] Função "${functionName}" encontrada com ID: ${ministryFunction.functionId}`,
          );
          ...
          this.logger.log(
            `[createUserAndMembershipFromSubmission] DTO criado: ${JSON.stringify(createMemberFunctionDto)}`,
          );
          ...
          this.logger.log(
            `[createUserAndMembershipFromSubmission] Função "${functionName}" criada com sucesso: ${JSON.stringify(result)}`,
          );
        } catch (functionError) {
          this.logger.error(
            `[createUserAndMembershipFromSubmission] Erro ao criar função "${functionName}": ${functionError.message}`,
          );
          this.logger.error(
            `[createUserAndMembershipFromSubmission] Stack trace: ${functionError.stack}`,
          );
      ...
      this.logger.warn(
        `[createUserAndMembershipFromSubmission] Não foi possível criar funções - selectedFunctions: ${submission.selectedFunctions}, preferredMinistry: ${submission.preferredMinistry}`,
      );
    }
