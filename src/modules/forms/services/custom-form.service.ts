...
export class CustomFormService {
  ...
  async createCustomForm(
    ...
    if (createDto.availableMinistries && createDto.availableMinistries.length > 0) {
      const ministries = await this.ministryModel.find({
        _id: { $in: createDto.availableMinistries.map(id => new Types.ObjectId(id)) },
        tenantId: new Types.ObjectId(tenantId),
        isActive: true,
      });

      if (ministries.length !== createDto.availableMinistries.length) {
        throw new BadRequestException('Alguns ministérios não foram encontrados ou não estão ativos');
      }
    }
    ...
    const formData = {
      ...
      availableMinistries: createDto.availableMinistries?.map(id => new Types.ObjectId(id)) || [],
  ...
  async getTenantForms(
    ...
    this.logger.log(`[getTenantForms] Buscando formulários - tenantId: ${tenantId}, branchId: ${branchId || 'null'}, page: ${page}, limit: ${limit}`);
    ...
    if (branchId) {
      ...
      this.logger.log(`[getTenantForms] Query com branch - incluindo formulários da branch ${branchId} e da matriz`);
    } else {
      query.branchId = null; // Apenas formulários da matriz
      this.logger.log(`[getTenantForms] Query sem branch - apenas formulários da matriz`);
    }

    this.logger.log(`[getTenantForms] Query executada: ${JSON.stringify(query)}`);
    ...
    this.logger.log(`[getTenantForms] Resultado - ${forms.length} formulários encontrados de ${total} total`);
    
    // Enriquecer campos de ministérios e funções para cada formulário
    this.logger.log(`[getTenantForms] Enriquecendo campos de ${forms.length} formulários`);
    const enrichedForms = await Promise.all(forms.map(async (form) => {
      form.fields = await this.enrichMinistryFields(form.fields, form.availableMinistries);
      return form;
    }));

    // Log detalhado de cada formulário encontrado
    enrichedForms.forEach((form, index) => {
      this.logger.log(`[getTenantForms] Formulário ${index + 1}: ${form._id}, título: ${form.title}, isActive: ${form.isActive}, branchId: ${form.branchId || 'null'}`);
    });
  ...
  async getFormById(formId: string, tenantId: string): Promise<CustomForm> {
    ...
    try {
      ...
      this.logger.log(`[getFormById] Query executada: ${JSON.stringify(query)}`);
      ...
      if (!form) {
        this.logger.warn(`[getFormById] Formulário não encontrado - formId: ${formId}, tenantId: ${tenantId}`);
        throw new NotFoundException('Formulário não encontrado');
      }

      this.logger.log(`[getFormById] Formulário encontrado: ${form._id}, título: ${form.title}, isActive: ${form.isActive}`);
      
      // Enriquecer campos de ministérios e funções
      this.logger.log(`[getFormById] Enriquecendo campos do formulário ${form._id}`);
      form.fields = await this.enrichMinistryFields(form.fields, form.availableMinistries);
      ...
      this.logger.error(`[getFormById] Erro na consulta do formulário ${formId}: ${error.message}`, error.stack);
      throw new NotFoundException('Formulário não encontrado');
    }
  }
  ...
  async getPublicForm(formId: string): Promise<CustomForm> {
    ...
    try {
      ...
      this.logger.log(`[getPublicForm] Query executada: ${JSON.stringify(query)}`);
      ...
      if (!form) {
        this.logger.warn(`[getPublicForm] Formulário público não encontrado - formId: ${formId}`);
        this.logger.warn(`[getPublicForm] Verificando se formulário existe com outros critérios...`);
        ...
        if (formExists) {
          this.logger.warn(`[getPublicForm] Formulário existe mas não atende critérios públicos:`);
          this.logger.warn(`[getPublicForm] - isPublic: ${formExists.isPublic}`);
          this.logger.warn(`[getPublicForm] - isActive: ${formExists.isActive}`);
          this.logger.warn(`[getPublicForm] - expiresAt: ${formExists.expiresAt}`);
          this.logger.warn(`[getPublicForm] - Data atual: ${new Date()}`);
        } else {
          this.logger.warn(`[getPublicForm] Formulário não existe no banco de dados`);
        }
        
        throw new NotFoundException('Formulário não encontrado ou expirado');
      }

      this.logger.log(`[getPublicForm] Formulário público encontrado: ${form._id}, título: ${form.title}, isPublic: ${form.isPublic}, expiresAt: ${form.expiresAt}`);
      
      // Enriquecer campos de ministérios e funções
      this.logger.log(`[getPublicForm] Enriquecendo campos do formulário ${form._id}`);
      form.fields = await this.enrichMinistryFields(form.fields, form.availableMinistries);
      ...
      this.logger.error(`[getPublicForm] Erro na consulta do formulário público ${formId}: ${error.message}`, error.stack);
      throw new NotFoundException('Formulário não encontrado ou expirado');
    }
  }
  ...
  async updateForm(
    ...
    // Validar ministérios se fornecidos
    if (updateDto.availableMinistries && updateDto.availableMinistries.length > 0) {
      const ministries = await this.ministryModel.find({
        _id: { $in: updateDto.availableMinistries.map(id => new Types.ObjectId(id)) },
        tenantId: new Types.ObjectId(tenantId),
        isActive: true,
      });
    ...
    const updateData = {
      ...updateDto,
      availableMinistries: updateDto.availableMinistries?.map(id => new Types.ObjectId(id)),
      expiresAt: updateDto.expiresAt ? new Date(updateDto.expiresAt) : undefined,
    };

    const updatedForm = await this.customFormModel.findByIdAndUpdate(
      formId,
      updateData,
      { new: true }
    ).populate('availableMinistries', 'name');
  ...
  async submitForm(
    ...
    this.logger.log(`[submitForm] Iniciando submissão do formulário: ${formId}`);
    this.logger.log(`[submitForm] Dados recebidos - email: ${submitDto.email}, nome: ${submitDto.volunteerName}`);

    const form = await this.getPublicForm(formId);
    this.logger.log(`[submitForm] Formulário validado: ${form._id}, allowMultipleSubmissions: ${form.settings.allowMultipleSubmissions}`);

    // Verificar se já existe submissão com este email (se não permitir múltiplas)
    if (!form.settings.allowMultipleSubmissions) {
      this.logger.log(`[submitForm] Verificando submissões existentes para email: ${submitDto.email}`);
      ...
      if (existingSubmission) {
        this.logger.warn(`[submitForm] Submissão já existe para email ${submitDto.email} no formulário ${formId}`);
        throw new BadRequestException('Você já submeteu este formulário');
      }
    }

    // Verificar se email já está cadastrado como usuário
    this.logger.log(`[submitForm] Verificando se email ${submitDto.email} já está cadastrado como usuário`);
    const existingUser = await this.userModel.findOne({ email: submitDto.email });
    if (existingUser) {
      this.logger.warn(`[submitForm] Email ${submitDto.email} já está cadastrado como usuário`);
    ...
    const filteredEssentialFields = Object.fromEntries(
      Object.entries(essentialFields).filter(([_, value]) => 
    ...
    const submissionData = {
      ...
      preferredMinistry: submitDto.preferredMinistry ? new Types.ObjectId(submitDto.preferredMinistry) : null,
      ...
      status: form.settings.requireApproval ? FormSubmissionStatus.PENDING : FormSubmissionStatus.APPROVED,
    };

    this.logger.log(`[submitForm] Criando submissão com status: ${submissionData.status}`);
    const submission = new this.formSubmissionModel(submissionData);
    await submission.save();
    this.logger.log(`[submitForm] Submissão criada com sucesso: ${submission._id}`);
    ...
    if (submission.preferredMinistry) {
      try {
        ...
        if (submitDto.selectedFunctions && submitDto.selectedFunctions.length > 0) {
          this.logger.log(`[submitForm] Criando ${submitDto.selectedFunctions.length} MemberFunctions para funções selecionadas`);
          this.logger.log(`[submitForm] Funções selecionadas: ${submitDto.selectedFunctions.join(', ')}`);
          
          // Buscar IDs das funções pelos nomes no ministério
          const { FunctionSchema } = await import('../../functions/schemas/function.schema');
          const { MinistryFunctionSchema } = await import('../../functions/schemas/ministry-function.schema');
          const functionModel = this.membershipModel.db.model('Function', FunctionSchema);
          ...
          for (const functionName of submitDto.selectedFunctions) {
            this.logger.log(`[submitForm] Buscando ID da função: ${functionName}`);
            ...
            try {
              ...
              if (!functionDoc) {
                this.logger.warn(`[submitForm] Função "${functionName}" não encontrada no tenant ${form.tenantId}`);
                continue;
              }
              ...
              if (!ministryFunction) {
                this.logger.warn(`[submitForm] Função "${functionName}" não está habilitada no ministério ${submission.preferredMinistry}`);
                continue;
              }
              
              this.logger.log(`[submitForm] Função "${functionName}" encontrada com ID: ${functionDoc._id}`);
              ...
              this.logger.log(`[submitForm] Dados da MemberFunction:`, memberFunctionData);
              ...
              this.logger.log(`[submitForm] MemberFunction criada com sucesso: ${memberFunction._id} para função: ${functionName} (ID: ${ministryFunction._id})`);
            } catch (memberFunctionError) {
              this.logger.error(`[submitForm] Erro ao criar MemberFunction para função ${functionName}: ${memberFunctionError.message}`);
              this.logger.error(`[submitForm] Stack trace:`, memberFunctionError.stack);
              // Não falhar a operação por erro na função
            }
          }
        } else {
          this.logger.warn(`[submitForm] Nenhuma função selecionada para criar MemberFunctions`);
        }

        this.logger.log(`[submitForm] Usuário, membership e funções criados com sucesso`);
        
      } catch (userCreationError) {
        this.logger.error(`[submitForm] Erro ao criar usuário/membership: ${userCreationError.message}`);
    ...
    this.logger.log(`[submitForm] Contador de submissões incrementado para formulário ${formId}`);

    // 🆕 Notificar líderes do ministério sobre nova submissão
    if (submission.preferredMinistry && submission.status === FormSubmissionStatus.PENDING) {
      try {
        ...
        this.logger.log(`[submitForm] Notificação enviada para líderes do ministério ${submission.preferredMinistry}`);
      } catch (notificationError) {
        this.logger.error(`[submitForm] Erro ao enviar notificação: ${notificationError.message}`);
  ...
  async getAllSubmissions(
    ...
    this.logger.log(`[getAllSubmissions] Buscando submissões - tenantId: ${tenantId}, status: ${status}, page: ${page}, limit: ${limit}`);
    ...
    if (status) {
      ...
      if (typeof status === 'string' && status.includes(',')) {
        ...
        this.logger.log(`[getAllSubmissions] Buscando múltiplos status: ${statusList}`);
      } else {
        query.status = status;
      }
    }
    ...
    this.logger.log(`[getAllSubmissions] Query executada: ${JSON.stringify(query)}`);
    ...
    this.logger.log(`[getAllSubmissions] Resultado - ${submissions.length} submissões encontradas de ${total} total`);
  ...
  async reviewSubmission(
    ...
    if (!this.isValidObjectId(submissionId)) {
      this.logger.error(`[reviewSubmission] SubmissionId inválido: ${submissionId}`);
    ...
    if (!this.isValidObjectId(reviewedBy)) {
      this.logger.error(`[reviewSubmission] ReviewedBy inválido: ${reviewedBy}`);
    ...
    const updateData = {
      status: reviewDto.status === 'approved' ? FormSubmissionStatus.APPROVED : FormSubmissionStatus.REJECTED,
  ...
  async getFormMinistries(formId: string): Promise<any[]> {
    ...
    if (!form) {
      this.logger.warn(`[getFormMinistries] Formulário não encontrado: ${formId}`);
      throw new BadRequestException('Formulário não encontrado');
    }

    this.logger.log(`[getFormMinistries] Formulário encontrado: ${form._id}, isPublic: ${form.isPublic}, availableMinistries: ${form.availableMinistries?.length || 0}`);

    if (!form.isPublic) {
      this.logger.warn(`[getFormMinistries] Formulário não é público: ${formId}`);
      throw new BadRequestException('Formulário não é público');
    }

    if (!form.availableMinistries || form.availableMinistries.length === 0) {
      this.logger.log(`[getFormMinistries] Nenhum ministério disponível para formulário: ${formId}`);
      return [];
    }

    this.logger.log(`[getFormMinistries] Buscando ${form.availableMinistries.length} ministérios ativos`);
    const ministries = await this.ministryModel.find({
      _id: { $in: form.availableMinistries },
      isActive: true,
    }).select('_id name description ministryFunctions');

    this.logger.log(`[getFormMinistries] Ministérios encontrados: ${ministries.length} para formulário ${formId}`);
  ...
  async getAllMinistries(): Promise<any[]> {
    ...
    const ministries = await this.ministryModel.find({
      isActive: true,
    }).select('_id name description ministryFunctions').sort({ name: 1 });
   ...
   */
  private async enrichMinistryFields(fields: any[], availableMinistries: Types.ObjectId[]): Promise<any[]> {
    this.logger.log(`[enrichMinistryFields] Enriquecendo ${fields.length} campos com ${availableMinistries.length} ministérios`);
    ...
    const enrichedFields = await Promise.all(fields.map(async (field) => {
      ...
      if (field.type === 'ministry_select') {
        this.logger.log(`[enrichMinistryFields] Enriquecendo campo ministry_select: ${field.id}`);
        
        let ministries;
        if (availableMinistries.length > 0) {
          // Usar ministérios específicos do formulário
          ministries = await this.ministryModel.find({
            _id: { $in: availableMinistries },
            isActive: true,
          }).select('_id name description');
          this.logger.log(`[enrichMinistryFields] Usando ${ministries.length} ministérios específicos do formulário`);
        } else {
          // Se não há ministérios específicos, usar todos os ministérios ativos
          ministries = await this.ministryModel.find({
            isActive: true,
          }).select('_id name description').sort({ name: 1 });
          this.logger.log(`[enrichMinistryFields] Usando todos os ${ministries.length} ministérios ativos`);
        }
        
        field.options = ministries.map(ministry => ({
          value: (ministry._id as any).toString(),
          label: ministry.name,
          description: ministry.description || ''
        }));
        
        this.logger.log(`[enrichMinistryFields] Campo ${field.id} enriquecido com ${field.options.length} ministérios`);
      }
      
      // Enriquecer campo de múltipla seleção de funções (campos dependentes)
      if (field.type === 'function_multiselect') {
        this.logger.log(`[enrichMinistryFields] Configurando campo function_multiselect dependente: ${field.id}`);
        ...
        this.logger.log(`[enrichMinistryFields] Campo ${field.id} configurado como dependente de 'ministerio'`);
      }
   ...
   */
  async getFormFunctions(formId: string, ministryIds: string[]): Promise<any[]> {
    this.logger.log(`[getFormFunctions] Buscando funções para formulário: ${formId}, ministryIds: ${JSON.stringify(ministryIds)}`);

    const form = await this.customFormModel.findById(formId);
    if (!form) {
      this.logger.warn(`[getFormFunctions] Formulário não encontrado: ${formId}`);
      throw new BadRequestException('Formulário não encontrado');
    }

    this.logger.log(`[getFormFunctions] Formulário encontrado: ${form._id}, isPublic: ${form.isPublic}`);

    if (!form.isPublic) {
      this.logger.warn(`[getFormFunctions] Formulário não é público: ${formId}`);
      throw new BadRequestException('Formulário não é público');
    }

    if (ministryIds.length === 0) {
      this.logger.log(`[getFormFunctions] Nenhum ministry ID fornecido para formulário: ${formId}`);
      return [];
    }

    // Validar se os ministérios estão disponíveis para este formulário
    const validMinistryIds = ministryIds.filter(id => 
      form.availableMinistries.some(availableId => availableId.toString() === id)
    );

    this.logger.log(`[getFormFunctions] Ministry IDs válidos: ${validMinistryIds.length}/${ministryIds.length} para formulário ${formId}`);

    if (validMinistryIds.length === 0) {
      this.logger.log(`[getFormFunctions] Nenhum ministry ID válido para formulário: ${formId}`);
      return [];
    }

    this.logger.log(`[getFormFunctions] Buscando funções de ${validMinistryIds.length} ministérios`);
    const ministries = await this.ministryModel.find({
      _id: { $in: validMinistryIds.map(id => new Types.ObjectId(id)) },
      isActive: true,
    }).select('_id name ministryFunctions');

    this.logger.log(`[getFormFunctions] Ministérios encontrados: ${ministries.length} para formulário ${formId}`);
    ...
    const result = ministries.map(ministry => ({
      ...
      functions: ministry.ministryFunctions ? ministry.ministryFunctions.map(func => ({
        value: func,
        label: func
      })) : []
    }));
    ...
    const allFunctionsArray = Array.from(allFunctions).sort().map(func => ({
      value: func,
      label: func
    }));

    this.logger.log(`[getFormFunctions] Retornando ${result.length} ministérios com funções e ${allFunctionsArray.length} funções únicas`);
   ...
   */
  private mergeWithDefaultFields(providedFields: any[], defaultFields: any[]): any[] {
