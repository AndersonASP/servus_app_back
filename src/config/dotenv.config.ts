...
export const loadEnvironmentFile = (): void => {
  ...
  if (result.error) {
    console.warn(
      '⚠️  Arquivo .env não encontrado na raiz. Usando apenas variáveis de ambiente do sistema.',
    );
    console.log(
...
export const validateRequiredEnvVars = (): void => {
  ...
  if (missing.length > 0) {
    console.error(
      `❌ Variáveis de ambiente obrigatórias não encontradas para ${nodeEnv}:`,
    );
    ...
    console.error(
      '💡 Para produção, configure as variáveis diretamente no serviço de hospedagem.',
    );
    process.exit(1);
  }
