...
async function createServusAdmin() {
  // Usar a mesma URI do .env
  const client = new MongoClient('mongodb://localhost:27017/servus_dev');
  
  try {
    await client.connect();
    console.log('🚀 Conectado ao MongoDB servus_dev');
    
    const db = client.db();
    
    console.log('🚀 Criando Servus Admin no banco correto...');
    console.log('========================');
    ...
    if (tenant && user && membership) {
      ...
      console.log('');
      console.log('✅ Servus Admin criado com sucesso no banco servus_dev!');
